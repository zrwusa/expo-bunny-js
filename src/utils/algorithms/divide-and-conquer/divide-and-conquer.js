...
const countSmallerBITPlagiarized = (nums) => {
    const ranks = new Map();
...
const countSmallerBIT = function (nums) {
    const orgNums = [...nums];
...
function smallestDistancePair(nums, k) {
    ...
    function countLessThan(nums, target) {
        let right = 0, count = 0;
        for (let i = 0; i < nums.length; i++) {
            while (nums[i] - nums[right] > target && right <= i)
                right++;
            count += (i - right);
        }
        return count;
    ...
    while (min < max - 1) {
        ...
        if (count > countLessThan(nums, mid - 1) && count === k)
            return mid;
        if (count < k) {
            min = mid;
        } else {
