...
export const runAlgorithm = async (algorithm, output = true, ...args) => {
...
export const isOneDiffOrdered = (wordA, wordB) => {
    let diffCount = 0;
...
export const isOneDiffOrderedPieced = (wordA, wordB) => {
    for (let i = 0, len = wordA.length; i < len; i++) {
        for (let j = 0; j < 26; j++) {
...
export const isOneDiff = (word1, word2) => {
    let diffCount = 0;
    for (let c1 of word1) {
...
const searchInSortedArray = function (nums, target) {
...
export const fourthQuadrantMoveByIndex = (departure, direction, matrix, judgeDeadOrDeadCells) => {
    const directions = {
        up: [-1, 0],
        right: [0, 1],
        down: [1, 0],
...
export const getRouteByParentsHash = (parents, leaf, hashFunction) => {
    const route = [leaf];
    const value = [leaf];
