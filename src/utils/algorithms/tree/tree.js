...
export async function letterCombinations(digits, proxyHandler) {
    ...
    if (digits.length === 0)
        return [];
    let proxyVariables = new DeepProxy({
        accumulated: '',
        result: []
    }, proxyHandler);
    const digitsMap = {
...
const permuteMN = function (nums, n, excludeSelf = true) {
    if (n > nums.length) {
        return [];
    }
    ...
    let result = [];
    const dfs = (accumulated, rest, level) => {
        if (level === n) {
            result.push([...accumulated]);
            return;
        }
...
// Combination
const combineMN = function (nums, n, excludeSelf = true) {
    if (n > nums.length) {
        return [];
    }
    if (nums.length === 1 && n === 1) {
...
export function ladderLengthDFS(beginWord, endWord, wordList, proxyHandler) {
    ...
    let shortest = 0;
    const dfs = (accumulated, rest, level, parentNode) => {
        // base case
...
export const ladderLengthBFS = function (beginWord, endWord, wordList) {
...
export const ladderLengthTwoWayBFS = function (beginWord, endWord, wordList) {
    if (wordList.length < 1 || !wordList.includes(endWord)) {
        return 0;
    }
    let queue1 = [beginWord];
    let queue2 = [endWord];
    let set1 = new Set(queue1);
    let set2 = new Set(queue2);
    let level = 1;
    let tempQueue = [];
...
export const updateMatrixByIndex = (mat) => {
    const rowCount = mat.length, colCount = mat[0].length;
    let departureQueue = [];
    let costMat = [];
    for (let y = 0; y < rowCount; y++) {
...
export async function cutOffTree(forest, proxyHandler) {
    ...
    if (treeCoordinates.length < 1)
        return -1;
    const sortedTreeCoordinates = treeCoordinates.sort((a, b) => forest[a.y][a.x] - forest[b.y][b.x]);
    let cost = 0;
    let directions = ['up', 'down', 'left', 'right'];
    const hashFunction = (cell) => cell.y + ',' + cell.x;
            ...
            for (let direction of directions) {
                ...
                if (destination && !visited[hashFunction(destination)]) {
                    ...
                    if (destination.y === to.y && destination.x === to.x) {
                        if (proxyVariables)
                            proxyVariables.cur = destination;
                        await wait(100);
...
function cutOffTreeByIndex(forest) {
    ...
    if (treeCoordinates.length < 1)
        return -1;
    const sortedTreeCoordinates = treeCoordinates.sort((a, b) => forest[a[0]][a[1]] - forest[b[0]][b[1]]);
    let cost = 0;
    let directions = ['up', 'down', 'left', 'right'];
    const hashFunction = (cell) => cell[0] + ',' + cell[1];
    const bfs = (from, to) => {
        let queue = [from];
        let level = 0;
        let tempQueue = [];
...
export const treeMaxDepth = (node) => {
    if (!node) {
        return 0;
    }
    const {children} = node;
