    ...
    return (<ScrollView style={[containerStyles.Screen]}>
        ...
        <View style={{paddingHorizontal: wp(10)}}>
            ...
            <Row style={{paddingHorizontal: wp(10), paddingVertical: wp(3)}}>
                ...
                <Col size={10}>
                    <Slider min={50} max={240} step={1} valueOnChange={value => setHeight(value)}
                            valueOnIndicated={(value) => setHeight(value)} initialValue={180} invert={false}
                            styleSize={24} showRangeLabels={false} showValueLabels={false}/>
                </Col>
        ...
        <ModalFromRight isVisible={isShowOccupationModal} onVisibleChanged={isVisible => {
            ...
            <TreeNodePicker titles={['Category', 'Occupation', 'Test']} keyExtractors={['code', 'code', 'code']}
                            dataMode="firestore" leafLevel={1} collectionPaths={['occupationCategories', 'occupations']}
                            conditions={[['', '==', ''], ['category', '==', '$code']]} displayFields={['name', 'name']}
                // dataMode="local"
        ...
        <ModalFromRight isVisible={isShowGraduateModal} onVisibleChanged={isVisible => {
            ...
            <TreeNodePicker titles={['University']} dataMode="firestore" leafLevel={0}
                            collectionPaths={['universities']} conditions={[['', '==', '']]} keyExtractors={['id']}
                            displayFields={['name']}
                // dataMode="local"
                // data={occupationTreeData}
                // childrenKeys={['children', 'children']}
        ...
        <ModalFromRight isVisible={isShowReligionModal} onVisibleChanged={isVisible => {
            ...
            <TreeNodePicker titles={['Religion']} dataMode="firestore" leafLevel={0} collectionPaths={['religions']}
                            conditions={[['', '==', '']]} keyExtractors={['id']} displayFields={['display']}
                // dataMode="local"
                // data={occupationTreeData}
                // childrenKeys={['children', 'children']}
                            onDone={(result) => {
        ...
        <ModalFromRight isVisible={isShowLiveInModal} onVisibleChanged={isVisible => {
            ...
            <TreeNodePicker titles={['Country', 'State', 'City']} dataMode="firestore" leafLevel={2}
                            collectionPaths={['countries', 'states', 'cities']}
                            conditions={[['', '==', ''], ['countryId', '==', '$id'], ['stateId', '==', '$id']]}
                            keyExtractors={['id', 'id', 'id']} displayFields={['name', 'name', 'name']}
                // dataMode="local"
                // data={occupationTreeData}
                // childrenKeys={['children', 'children']}
                            onDone={(result) => {
