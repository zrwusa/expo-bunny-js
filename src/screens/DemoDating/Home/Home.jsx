...
export function DatingHomeScreen() {
    ...
    return (<View style={styles.screen}>
            ...
            <Row style={styles.albumContainer}>
                <CardStack style={styles.album} loop verticalSwipe={true} renderNoMoreCards={() => null} ref={swipper} horizontalThreshold={60} onSwipedLeft={() => {
        }} onSwipedRight={() => {
        }}>
                    {data.map((item) => (<StackSwiperCard key={item.id}>
                            <CardItem hasActions image={item.image} name={item.name} description={item.description} matches={item.match}/>
                        </StackSwiperCard>))}
                </CardStack>
            </Row>
        </View>);
