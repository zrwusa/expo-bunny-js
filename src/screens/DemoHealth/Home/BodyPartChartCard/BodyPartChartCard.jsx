...
export function BodyPartChartCard(props) {
    ...
    return <View style={styles.bodyPartChartCard}>
        ...
        <Row>
            <VictoryChart width={wp(326)} height={160} theme={victory} padding={{ top: wp(10), left: wp(42), bottom: wp(42), right: wp(20) }} 
    // animate={{
    //     duration: 1000,
    // }}
    domainPadding={{ y: wp(15) }} containerComponent={<VictoryVoronoiContainer voronoiDimension="x" labels={({ datum }) => `x:${datum.x.toLocaleDateString()} \n y: ${datum.y}`} labelComponent={<VictoryTooltip constrainToVisibleArea cornerRadius={ms.br.s}/>}/>} scale={{ x: 'time' }}>
            ...
            ticks: { stroke: 'none' },
            axis: { stroke: 'none' },
        }} axisLabelComponent={<VictoryLabel dy={wp(22)} dx={wp(0)}/>} label="Time (week)"/>
                <VictoryAxis dependentAxis axisLabelComponent={<VictoryLabel dy={wp(-26)} dx={wp(-10)}/>} label="Max Weight (kgs)" tickCount={4} style={{
            grid: { stroke: 'none' },
            ticks: { stroke: 'none' },
            axis: { stroke: 'none' },
        }}/>
