...
export function HealthHomeScreen() {
    ...
    const monthKeys = Object.keys(E_MONTH);
    ...
    const data1Mapped = data1.map((item, index) => {
        return {x: new Date(item.x), y: parseFloat((item.y / 1000).toFixed(0))};
    });
    const data2Mapped = data2.map((item, index) => {
        return {x: new Date(item.x), y: parseFloat((item.y / 1000).toFixed(0))};
    });
            ...
            <View style={[containerStyles.Screen]}>
                <View style={{padding: wp(16)}}>
                    ...
                    <Row>
                        ...
                        <Col>
                            <PickerSelect items={monthItems} value="January" onValueChange={(value) => {
