...
function App() {
    ...
    return isReady
        ...
        <AppearanceProvider>
                ...
                return <>
                                                <StatusBar backgroundColor={Platform.OS === 'android' ? theme.colors.background : ''} barStyle={theme.dark ? 'light-content' : 'dark-content'}/>
                                                <NavigatorTree theme={theme} fallback={<Text>{t(`sys.navigationFallback`)}</Text>} initialState={navInitialStateMemorized} onStateChange={(state) => AsyncStorage.setItem(BunnyConstants.NAV_STATE_PERSISTENCE_KEY, JSON.stringify(state))}/>
                                                <Sys />
                                                <RequestLoading />
                                                <BLToast />
                                            </>;
