...
import {ModalFromRight} from '../../containers';
            ...
            <View style={{flex: 1, marginBottom: wp(10)}}>
                ...
                {(dataMode === 'local' && children) || (dataMode === 'firestore' && children)
                    ? <ModalFromRight isVisible={isShowChildrenModal} onVisibleChanged={isVisible => {
                        ...
                        <TreeNodePicker condition={childrenCondition} data={dataMode === 'local' ? children : undefined}
                                        level={childrenLevel} onDone={(result) => {
                            setIsShowChildrenModal(false);
                            onDone?.(result);
                        }} onCancel={() => {
                            setIsShowChildrenModal(false);
                        }} currentNodes={currentNodesState} leafLevel={leafLevel} dataMode={dataMode} titles={titles}
                                        collectionPaths={collectionPaths} conditions={conditions}
                                        keyExtractors={keyExtractors} initialTreeNode={initialTreeNode}
