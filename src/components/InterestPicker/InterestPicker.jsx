...
export const InterestPicker = (props) => {
    ...
    return (<View style={[styles.container]}>
        ...
        <Modal isVisible={showModal} propagateSwipe={true} onSwipeComplete={() => setShowModal(false)}
               swipeDirection="down" style={styles.modal} onBackdropPress={() => setShowModal(false)}>
            <View style={styles.modalContainer}>
                ...
                <View style={styles.content}>
                    <FlatList style={{height: wp(400)}} data={dictionaries[modalType]} keyExtractor={item => item.code}
                              renderItem={({item}) => {
                                  return <Checkbox.Item labelStyle={sharedStyles.text2} label={item.name}
                                                        color={colors.primary} status={checkState[item.code]}
                                                        onPress={() => {
