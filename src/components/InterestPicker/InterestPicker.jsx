...
export const InterestPicker = (props) => {
    ...
    const { onDone, type = 'edit', initialSports = [{ name: 'ATV', code: 'A0001' }], initialMusics = [], initialFoods = [], initialMovies = [], initialBooks = [], initialTravels = [], } = props;
    ...
    const [modalType, setModalType] = useState('musics');
    const [checkState, setCheckState] = useState({});
    const _toggleModal = (type) => {
        setModalType(type);
        let newCheckState = {};
    ...
    return (<View style={[styles.container]}>
            <View>
                <InlineSelector title="Sports" onPress={() => {
            _toggleModal('sports');
        }} renderText={() => result.sports.map(item => item.name).join(',')}/>
                <Divider />
                <InlineSelector title="Musics" onPress={() => {
            _toggleModal('musics');
        }} renderText={() => result.musics.map(item => item.name).join(',')}/>
                <Divider />
                <InlineSelector title="Foods" onPress={() => {
            _toggleModal('foods');
        }} renderText={() => result.foods.map(item => item.name).join(',')}/>
                <Divider />
                <InlineSelector title="Movies" onPress={() => {
            _toggleModal('movies');
        }} renderText={() => result.movies.map(item => item.name).join(',')}/>
                <Divider />
                <InlineSelector title="Books" onPress={() => {
            _toggleModal('books');
        }} renderText={() => result.books.map(item => item.name).join(',')}/>
                <Divider />
                <InlineSelector title="Travels" onPress={() => {
            _toggleModal('travels');
        }} renderText={() => result.travels.map(item => item.name).join(',')}/>
            </View>

            <Modal isVisible={showModal} propagateSwipe={true} onSwipeComplete={() => setShowModal(false)} swipeDirection="down" style={styles.modal} onBackdropPress={() => setShowModal(false)}>
            ...
            setShowModal(false);
                            ...
                            <Col align="flex-end"><TextButton onPress={_reset}><Text style={[sharedStyles.text2, {
                ...
                paddingHorizontal: wp(5)
                    ...
                    <View style={styles.content}>
                        <FlatList style={{ height: wp(400) }} data={dictionaries[modalType]} keyExtractor={item => item.code} renderItem={({ item }) => {
            return <Checkbox.Item labelStyle={sharedStyles.text2} label={item.name} color={colors.primary} status={checkState[item.code]} onPress={() => {
