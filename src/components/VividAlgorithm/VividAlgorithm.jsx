...
import { AbstractGraph, BinaryTree, BinaryTreeNode, Coordinate, DirectedGraph, getDirectionVector, SinglyLinkedListNode, Stack, UndirectedGraph, uuidV4 } from '../../utils';
import { TreeNode } from '../../types';
import Svg, { Circle, Defs, G, Line, Marker, Path, Rect, Text as SVGText, TSpan } from 'react-native-svg';
import { ScrollView } from 'react-native';
export const VividAlgorithm = function (props) {
    ...
    let relatedNode;
    let relatedBinaryNode;
    let relatedMatrixCell;
    if (relatedNodeKey) {
        relatedNode = data?.[relatedNodeKey];
        relatedBinaryNode = data?.[relatedNodeKey];
        relatedMatrixCell = data?.[relatedNodeKey];
    }
    // TODO render bug needs to be fixed
    let relatedMatrixRoutes;
    if (relatedRouteKey) {
        relatedMatrixRoutes = data?.[relatedRouteKey];
    }
    const VividNumber = ({ data }) => {
        return (<Row>
                <Col size={6}><Text>{data.toString()}</Text></Col>
            </Row>);
    };
    const getPointsByDelta = (src, dest, cutDelta) => {
        if (cutDelta === undefined)
            cutDelta = 0;
        const PI = Math.PI;
        let angle = Math.atan2((dest.y - src.y), (dest.x - src.x));
    ...
    const LineWithArrow = ({ from, to, weight, delta }) => {
        if (delta === undefined)
            delta = 0;
        const { src, dest } = getPointsByDelta(from, to, delta);
        return <G>
            <Defs>
                <Marker id="Triangle" viewBox="0 0 10 10" refX="0" refY="5" markerWidth="4" markerHeight="3" orient="auto">
                    <Path d="M 0 0 L 10 5 L 0 10 z" fill={arrowColor} stroke={arrowColor}/>
                </Marker>
            </Defs>
            <Path d={`M ${src.x} ${src.y} L ${dest.x} ${dest.y}`} fill={arrowColor} stroke={arrowColor} strokeWidth="2" markerEnd="url(#Triangle)"/>
            {weight !== undefined && weight !== null
                ? <SVGText strokeWidth={wp(1)} fontSize={ms.fs.xs} fill={colors.text} fontWeight={100} stroke={colors.text} x={src.x + (dest.x - src.x) / 2 + (src.x > dest.x ? wp(10) : wp(-10))} y={src.y + (dest.y - src.y) / 2 + (src.y > dest.y ? wp(3) : wp(-3))} textAnchor="middle">{weight}</SVGText>
                : null}

        </G>;
    };
    const VividString = ({ data }) => {
        return (<Row>
                <Col size={6}><Text>{data}</Text></Col>
            </Row>);
    };
    const TwoWayScrollSVG = ({ children, autoScroll }) => {
        ...
        return (<ScrollView nestedScrollEnabled style={{ height: wp(375) }}>
                <ScrollView style={{ width: wp(375) }} horizontal ref={horizontalScrollView}>
                    <Svg width={treePanelWidth} height={treePanelHeight}>
    ...
    const VividTree = ({ data }) => {
        return (<TwoWayScrollSVG autoScroll>
                <VividTreeRecursive node={data} level={1} index={0} familyLength={1} parentX={0} parentY={0} maxHeight={data.getHeight()}/>
            </TwoWayScrollSVG>);
    };
    const VividBinaryTree = ({ node, maxHeight }) => {
        return (<TwoWayScrollSVG autoScroll>
                {node
                ? <VividBinaryTreeRecursive node={node} level={1} index={0} familyLength={1} maxHeight={maxHeight}/>
                : null}
            </TwoWayScrollSVG>);
    };
    const VividGraph = ({ data }) => {
        return (<TwoWayScrollSVG autoScroll={false}>
                {data
    ...
    const VividMatrix = ({ data }) => {
        ...
        return (<Svg width={matrixPanelWidth} height={matrixHeight}>
                <G>
                    {data.map((row, i) => {
                return row.map((col, j) => {
                    const colKey = i + '-' + j.toString();
                    const isActive = (relatedMatrixCell?.y === i && relatedMatrixCell?.x === j);
                    return <Rect key={colKey} x={j * rectSize} y={i * rectSize} width={rectSize} height={rectSize} stroke={colors.border} strokeDasharray={`${rectSize},${rectSize * 2},${rectSize}`} strokeWidth={matrixRectStrokeWidth} fill={isActive ? colors.primary : colors.backgroundA}/>;
                });
            })}
                    {data.map((row, i) => {
                ...
                return row.map((col, j) => {
                    ...
                    const isActive = (relatedMatrixCell?.y === i && relatedMatrixCell?.x === j);
                    return <SVGText key={colKey} strokeWidth={wp(1)} fontSize={ms.fs.m} fill={isActive ? colors.buttonText : colors.text} fontWeight={100} x={(j + 0.5) * rectSize} y={(i + 0.5) * rectSize} textAnchor="middle">{data[i][j].toString()}</SVGText>;
                });
            })}
                    {relatedMatrixRoutes
                ? relatedMatrixRoutes.map((route, routeIndex) => {
                    return route.map((cell, cellIndex) => {
                        ...
                        if (from && to) {
                            ...
                            return <LineWithArrow key={src.y + ',' + src.x + dest.y + dest.x} from={src} to={dest}/>;
                        }
                        else {
                            return null;
                        }
    ...
    const VividTreeRecursive = ({ node, level = 1, index = 0, familyLength = 1, parentX, parentY, maxHeight }) => {
        if (!node) {
            return null;
        }
        ...
        return (<G key={node.id}>
                ...
                : null}
                {node.children
                ? node.children.map((child, index, family) => <VividTreeRecursive key={child.id} node={child} level={level + 1} index={index} familyLength={family.length} parentX={offsetX} parentY={offsetY} maxHeight={maxHeight}/>)
                : null}
                <Circle r={circleR} cx={offsetX} cy={offsetY} fill={isActive ? colors.primary : colors.background}/>
                <SVGText strokeWidth={wp(1)} fill={isActive ? colors.buttonText : colors.text} stroke={isActive ? colors.buttonText : colors.text} fontSize={fontSize} fontWeight={100} x={offsetX} y={offsetY + fontOffsetY} textAnchor="middle">{node.name || node.id}</SVGText>
            </G>);
    };
    const VividBinaryTreeRecursive = ({ node, level = 1, index = 0, familyLength = 1, parentX, parentY, maxHeight }) => {
        if (!node) {
            return null;
        }
        let space = 0;
        ...
        return (<G key={node.id}>
                ...
                {node.left
                ?
                    <VividBinaryTreeRecursive node={node.left} level={level + 1} index={0} familyLength={2} parentX={offsetX} parentY={offsetY} maxHeight={maxHeight}/>
                : null}
                {node.right
                ?
                    <VividBinaryTreeRecursive node={node.right} level={level + 1} index={1} familyLength={2} parentX={offsetX} parentY={offsetY} maxHeight={maxHeight}/>
                : null}
                <Circle r={circleR} cx={offsetX} cy={offsetY} fill={isActive ? colors.primary : colors.background}/>
                <SVGText fill="none" stroke={isActive ? colors.buttonText : colors.text} fontSize={fontSize} fontWeight={1} x={offsetX} y={offsetY + fontOffsetY} textAnchor="middle">
                    <TSpan x={offsetX} y={offsetY + fontOffsetY}>{node.id}</TSpan>
                    <TSpan x={offsetX} y={offsetY + fontOffsetY + fontSize + wp(2)}>{node.count}</TSpan>

                    <TSpan x={offsetX} y={offsetY + fontOffsetY + 2 * fontSize + wp(4)}>{node.allLesserSum}</TSpan>

                </SVGText>
            </G>);
    };
    const VividGraphDrawer = ({ graph }) => {
        ...
        return (<G>
                ...
                return (coordinate
                    ? <G key={vertex.id}>
                                    <Circle key={vertex.id} r={circleR} cx={coordinate.x} cy={coordinate.y} fill={colors.primary}/>
                                    <SVGText key={vertex.id + 'id'} fill="none" stroke={colors.text} fontSize={fontSize} fontWeight={1} x={coordinate.x} y={coordinate.y + fontOffsetY} textAnchor="middle">
                ...
                if (graph instanceof UndirectedGraph) {
                    ...
                    if (src && dest) {
                        ...
                        if (srcCod && destCod) {
                            return <LineWithArrow key={edge.hashCode} from={srcCod} to={destCod} weight={edge?.weight} delta={circleR}/>;
                        }
                    }
                }
            })}
    ...
    const VividLinkedListNode = ({ data }) => {
        return (<Row>
    ...
    // TODO
    const VividBinaryTreeNode = ({ data }) => {
        return (<Row>
    ...
    const VividBinarySearchTreeNode = ({ data }) => {
        return (<Row>
                <View style={styles.arrayItem} key={data.id}>
                    <Text>{data.id}</Text>
                </View>
