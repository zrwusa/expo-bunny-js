...
export const VividAlgorithm = function (props) {
    ...
    const LineWithArrow = ({from, to, weight, delta}) => {
        ...
        return <G>
            <Defs>
                <Marker id="Triangle" viewBox="0 0 10 10" refX="0" refY="5" markerWidth="4" markerHeight="3"
                        orient="auto">
                    <Path d="M 0 0 L 10 5 L 0 10 z" fill={arrowColor} stroke={arrowColor}/>
                </Marker>
            </Defs>
            <Path d={`M ${src.x} ${src.y} L ${dest.x} ${dest.y}`} fill={arrowColor} stroke={arrowColor} strokeWidth="2"
                  markerEnd="url(#Triangle)"/>
            {weight !== undefined && weight !== null
                ? <SVGText strokeWidth={wp(1)} fontSize={ms.fs.xs} fill={colors.text} fontWeight={100}
                           stroke={colors.text} x={src.x + (dest.x - src.x) / 2 + (src.x > dest.x ? wp(10) : wp(-10))}
                           y={src.y + (dest.y - src.y) / 2 + (src.y > dest.y ? wp(3) : wp(-3))}
                           textAnchor="middle">{weight}</SVGText>
                : null}
            ...
            <G>
                {data.map((row, i) => {
                    return row.map((col, j) => {
                        ...
                        return <Rect key={colKey} x={j * rectSize} y={i * rectSize} width={rectSize} height={rectSize}
                                     stroke={colors.border} strokeDasharray={`${rectSize},${rectSize * 2},${rectSize}`}
                ...
                {data.map((row, i) => {
                    ...
                    return row.map((col, j) => {
                        ...
                        const isActive = (relatedMatrixCell?.y === i && relatedMatrixCell?.x === j);
                        return <SVGText key={colKey} strokeWidth={wp(1)} fontSize={ms.fs.m}
                                        fill={isActive ? colors.buttonText : colors.text} fontWeight={100}
                                        x={(j + 0.5) * rectSize} y={(i + 0.5) * rectSize}
                                        textAnchor="middle">{data[i][j].toString()}</SVGText>;
                    });
                })}
                {relatedMatrixRoutes
                    ? relatedMatrixRoutes.map((route, routeIndex) => {
                        return route.map((cell, cellIndex) => {
                            ...
                            if (from && to) {
                                ...
                                return <LineWithArrow key={src.y + ',' + src.x + dest.y + dest.x} from={src}
                                                      to={dest}/>;
                            } else {
                                return null;
                            }
    ...
    const VividTreeRecursive = ({node, level = 1, index = 0, familyLength = 1, parentX, parentY, maxHeight}) => {
        ...
        return (<G key={node.id}>
            {level > 1
                ...
                : null}
            {node.children
                ? node.children.map((child, index, family) => <VividTreeRecursive key={child.id} node={child}
            ...
            <SVGText strokeWidth={wp(1)} fill={isActive ? colors.buttonText : colors.text}
                     stroke={isActive ? colors.buttonText : colors.text} fontSize={fontSize} fontWeight={100}
                     x={offsetX} y={offsetY + fontOffsetY} textAnchor="middle">{node.name || node.id}</SVGText>
        </G>);
    };
    const VividBinaryTreeRecursive = ({node, level = 1, index = 0, familyLength = 1, parentX, parentY, maxHeight}) => {
        ...
        return (<G key={node.id}>
            ...
            {node.left
                ?
                <VividBinaryTreeRecursive node={node.left} level={level + 1} index={0} familyLength={2}
                                          parentX={offsetX} parentY={offsetY} maxHeight={maxHeight}/>
                : null}
            {node.right
                ?
                <VividBinaryTreeRecursive node={node.right} level={level + 1} index={1} familyLength={2}
                                          parentX={offsetX} parentY={offsetY} maxHeight={maxHeight}/>
                : null}
            <Circle r={circleR} cx={offsetX} cy={offsetY} fill={isActive ? colors.primary : colors.background}/>
            <SVGText fill="none" stroke={isActive ? colors.buttonText : colors.text} fontSize={fontSize} fontWeight={1}
                     x={offsetX} y={offsetY + fontOffsetY} textAnchor="middle">
            ...
            {[...vertices].map(([id, vertex]) => {
                ...
                return (coordinate
                    ? <G key={vertex.id}>
                        ...
                        <SVGText key={vertex.id + 'id'} fill="none" stroke={colors.text} fontSize={fontSize}
                                 fontWeight={1} x={coordinate.x} y={coordinate.y + fontOffsetY} textAnchor="middle">
            ...
            {edges.map(edge => {
                if (graph instanceof UndirectedGraph) {
                    ...
                    if (src && dest) {
                        ...
                        if (srcCod && destCod) {
                            return <LineWithArrow key={edge.hashCode} from={srcCod} to={destCod} weight={edge?.weight}
                                                  delta={circleR}/>;
                        }
                    }
                }
