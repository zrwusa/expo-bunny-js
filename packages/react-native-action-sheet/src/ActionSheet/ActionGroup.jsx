...
export default class ActionGroup extends React.Component {
    constructor() {
        super(...arguments);
        this._renderRowSeparator = (key) => {
            return (<View key={`separator-${key}`} style={[styles.rowSeparator, this.props.separatorStyle]}/>);
        };
        ...
        this._renderOptionViews = () => {
            const { options, icons, destructiveButtonIndex, destructiveColor = DESTRUCTIVE_COLOR, onSelect, startIndex, length, textStyle, tintColor, autoFocus, showSeparators, } = this.props;
            const optionViews = [];
            const nativeFeedbackBackground = TouchableNativeFeedbackSafe.Ripple('rgba(180, 180, 180, 1)', false);
            for (let i = startIndex; i < startIndex + length; i++) {
                ...
                optionViews.push(<TouchableNativeFeedbackSafe ref={autoFocus && i === 0 ? focusViewOnRender : undefined} key={i} pressInDelay={0} background={nativeFeedbackBackground} onPress={() => onSelect(i)} style={styles.button} accessibilityRole="button" accessibilityLabel={options[i]}>
                    {this._renderIconElement(iconSource, color)}
    ...
    render() {
        return (<View style={[styles.groupContainer, this.props.containerStyle]}>
                {this._renderTitleContent()}
