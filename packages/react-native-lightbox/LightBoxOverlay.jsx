...
export class LightBoxOverlay extends Component {
    constructor(props) {
        ...
        this.open = () => {
            ...
            Animated.spring(this.state.openVal, { toValue: 1, ...this.props.springConfig, useNativeDriver: false }).start(() => {
        ...
        this.close = () => {
            ...
            Animated.spring(this.state.openVal, { toValue: 0, ...this.props.springConfig, useNativeDriver: false }).start(() => {
                this.setState({
                    isAnimating: false,
                });
        ...
        this._panResponder = PanResponder.create({
            ...
            onPanResponderRelease: (evt, gestureState) => {
                if (Math.abs(gestureState.dy) > DRAG_DISMISS_THRESHOLD) {
                    ...
                    Animated.spring(this.state.pan, { toValue: 0, ...this.props.springConfig, useNativeDriver: false }).start(() => {
    ...
    render() {
        ...
        const content = (<Animated.View style={[openStyle, dragStyle]} {...handlers}>
                {this.props.children}
            </Animated.View>);
        ...
        return (<Modal visible={isOpen} transparent={true} onRequestClose={() => this.close()}>
                {background}
                {content}
                {header}
            </Modal>);
